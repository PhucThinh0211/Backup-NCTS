...
export const BannerListTable = () => {
  ...
  useEffect(() => {
    if (banners) {
      let sortedBanners: BannerResponse[] = Utils.deepClone(
        banners?.items || []
      );
      sortedBanners.sort((a, b) => a.sortSeq - b.sortSeq);
      if (queryParams?.search) {
        sortedBanners = sortedBanners.filter((item) => {
          return (
            (item.title || "")
              .toLowerCase()
